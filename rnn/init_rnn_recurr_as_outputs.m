function net = init_rnn_recurr_as_outputs(layer_sizes, layer_types, g, obj_fun_type, varargin)
...
% Keep output units identical to a subset of recurrent units. This way the output units are embedded into
% the recurrent layer. - H.M.
...
for i = 1:nlayers
    ...
    if i == 3
        W{i} = eye(npost, npre);  % output units are identical to the first npost recurrent units
    else
...
mod_mask = zeros(nparams,1);   % Default is that all parameters are not actually parameters.
for i = 1:nlayers
    ...
    if i == 1
        cost_mask(mask_start_idx:mask_stop_idx) = cost_mask_layer_fac(i);  % penalty on input weights
        mod_mask(mask_start_idx:mask_stop_idx)  = mod_mask_layer_fac(i);   % learn input weights
    elseif i == 2
        mod_mask(mask_start_idx:(mask_stop_idx+npost)) = mod_mask_layer_fac(i);  % learn both weights and bias
        cost_mask(mask_start_idx:mask_stop_idx) = cost_mask_layer_fac(i);  % penalty on recurrent weights
    end
